...
class SegRandomCropData():
    ...
    def __call__(self, data: dict) -> dict:
        """
        :param data: {'img':,'seg_polys':,'ignore_tags':}
        :return:
        """
        im = data['img']
        seg_polys = data['seg_polys']
        ignore_tags = data['ignore_tags']
        all_care_polys = [seg_polys[i] for i, tag in enumerate(ignore_tags) if not tag]
        # 计算crop区域
        ...
        seg_polys_crop = []
        ignore_tags_crop = []
        for poly, tag in zip(seg_polys, ignore_tags):
            poly = ((poly - (crop_x, crop_y)) * scale).tolist() 
            if not self.is_poly_outside_rect(poly, 0, 0, w, h):
                if not self.is_poly_in_rect(poly, 0, 0, w, h):
                    tag = True
                seg_polys_crop.append(poly)
                ignore_tags_crop.append(tag)

        data['img'] = img
        data['seg_polys'] = np.float32(seg_polys_crop)
    ...
    def crop_area(self, im, seg_polys):
        h, w = im.shape[:2]
        h_array = np.zeros(h, dtype=np.int32)
        w_array = np.zeros(w, dtype=np.int32)
        for points in seg_polys:
        ...
        for i in range(self.max_tries):
            ...
            for poly in seg_polys:
                if self.is_poly_in_rect(poly, xmin, ymin, xmax - xmin, ymax - ymin):
