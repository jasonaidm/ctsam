...
class ResNetMed3D(BaseModel):
    ...
    def __init__(self, in_channels=3, num_classes=10,
        ...
        self.head = ClsHead(in_channels=block_inplanes[3]*block.expansion, num_classes=num_classes)
    ...
    def test(self):
        ...
        y = self.forward(a)
        target = torch.rand(1, self.num_classes, 16, 16, 16)
        assert a.shape == y.shape


def generate_resnet3d(in_channels=1, num_classes=2, model_depth=18, **kwargs):
    ...
    if model_depth == 10:
        model = ResNetMed3D(in_channels=in_channels, num_classes=num_classes, block=BasicBlock,
        ...
        model = ResNetMed3D(in_channels=in_channels, num_classes=num_classes, block=Bottleneck,
                            layers=res_net_dict[model_depth], block_inplanes=in_planes, **kwargs)
...
if __name__ == '__main__':
    ...
    model = generate_resnet3d(in_channels=1, num_classes=2, model_depth=18).cuda()
    a = torch.rand(1, 1, 64, 512, 512).cuda()
    y = model(a)
    print(y)
